...
export default props => {
  ...
  if (props.licences.length === 0) return <p className="mb-5">No matching water accounts found</p>;

  return (
    <table className="mb-5 border bg-white">
      <thead>
        ...
        {props.licences.map(l => {
          ...
          return (
            <tr key={l.accountId} className={classNames(classes)}>
              ...
              <td className="p-2">
                {!props.unclaimed && !isActiveLicence && (
                  <button
                ...
                {props.unclaimed && (
                  <button className="text-sm text-green-600" onClick={() => dispatch(claimWaterAccountsForLicence(l))}>
